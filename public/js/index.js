document.addEventListener('DOMContentLoaded', () => {
    ...
    async function getAllPolls(){
        const response = await fetch(`http://localhost:8080/api/polls`); /* use relative path */
        const data = await response.json()
        return data;
    }
    async function getPollBySearch(Search){
        const response = await fetch(`http://localhost:8080/api/polls/search/${Search}`); /* use relative path */
        const data = await response.json()
            return data;
    } 
